                         ...
                         std::function<void (QSharedPointer<ValueEditor::Model>, const QString &)> callback)
{
                                 ...
                                 (RedisClient::Response resp, const QString& e)
    {
        ...
        if (resp.isErrorMessage() || resp.getType() != RedisClient::Response::Type::Status || !e.isEmpty()) {
            QString msg("Cannot load key %1, connection error occurred: %2");
            callback(result, msg.arg(printableString(keyFullPath)).arg(e.isEmpty()? resp.toRawString() : e));
            return;
        }
